...
import burpgpt.gpt.diyGPTResponse;
...
public class GPTClient {
  ...
  public Pair<GPTRequest, diyGPTResponse> identifyVulnerabilities(HttpRequestResponse selectedMessage) throws IOException {
    ...
    try {
      ...
      diyGPTResponse gptResponse = getCompletions(gptRequest, apiKey, model, prompt);
  ...
  private diyGPTResponse getCompletions(GPTRequest gptRequest, String apiKey, String model, String prompt)
      throws IOException {
    gptRequest.setPrompt(prompt);

    String apiEndpoint = "https://api.chatanywhere.tech/v1/chat/completions";
    ...
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) {
        ...
        return gson.fromJson(responseBody, diyGPTResponse.class);
      }
    } catch (IOException e) {
